import { DomainEvent } from '@src/bounded-contexts/shared/domain/domain-event.interface';

interface Props {
  orderId: string;
  previousStatus: string;
  newStatus: string;
}

export class OrderCompletedEvent extends DomainEvent<Omit<Props, 'orderId'>> {
  static readonly EVENT_NAME = 'order.status.completed';
  static readonly EVENT_VERSION = 1;

  constructor(props: Props) {
    const { orderId, previousStatus, newStatus } = props;

    super(
      OrderCompletedEvent.EVENT_NAME,
      orderId,
      OrderCompletedEvent.EVENT_VERSION,
      {
        previousStatus,
        newStatus,
      }
    );
  }
}
